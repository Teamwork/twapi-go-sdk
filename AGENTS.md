...
- **Document a group or option a shared type offers but the endpoint drops.** A request struct reusing a shared type advertises every group in it, and the endpoint says nothing about the one it ignored, so a caller reads the type and believes the assignment happened. `LegacyUserGroups` carries `JobRoleIDs` for the notify groups, but the milestone routes never apply it: `MilestoneCreateRequest` is answered 422 `Invalid milestone assignees` for a job-role-only list, `MilestoneUpdateRequest` is answered 200 with the assignees untouched, and either one discards a job role sent alongside a user. State it on the field that carries the type, not only on the type — the type cannot know which endpoint is reading it.
